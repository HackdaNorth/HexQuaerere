            ...
            if all(byte != "00" for byte in segment) and \
               ...
               all(byte != "80" for byte in segment):
                if segment in unique_segments:
                    ...
                    hex_address1 = "0x{:05x}".format(ram_address1 * 2)  # Assuming each RAM address maps to 2 hex addresses
                    successful_matches.append((line1_idx + 1, ram_address1, hex_address1, matched_bytes))
                    print(f"    Match found: Pairs1 Line {line1_idx + 1}, RAM Address {ram_address1}, HEX Address {hex_address1} , Matched Bytes: {matched_bytes}")
...
def save_matches_to_file(matches, output_file):
    with open(output_file, 'w') as file:
        for match in matches:
            ...
            file.write(f"Pairs1 Line {match[0]}, RAM Address 0x{match[1]:05x}, HEX Address {match[2]}:\n")
            file.write(f"Matched Bytes: {match[3]}\n\n")

def main():
    pairs1_path = "pairs1.txt"
    pairs2_path = "pairs2.txt"
    output_file = "matches_direct_with_hexv2.txt"
