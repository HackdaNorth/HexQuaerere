                    ...
                    for line2_idx, line2_pairs in enumerate(pair_list2):
                        for j in range(len(line2_pairs) - 3):
                            if segment == tuple(line2_pairs[j:j+4][::-1]):
                                hex_offset2 = "0x{:08x}".format((line2_idx * 4 + j) * 2)  # Calculate HEX offset for pairs2
                                successful_matches.append((line1_idx + 1, hex_offset1, line2_idx + 1, hex_offset2, matched_bytes))
...
def save_matches_to_file(matches, output_file):
    with open(output_file, 'w') as file:
        for match in matches:
            ...
            file.write(f"Matched Bytes: Pairs1: {match[4]} <->  Pairs2: {match[5]}\n\n")

def main():
    pairs1_path = "pairs1.txt"
    pairs2_path = "pairs2.txt"
    output_file = "matches_with_hex_offset_v2.txt"
